...
int minimax(char board[3 * 3], int depth, int alpha, int beta, int maximizing_player){  //for maximizing player, 1 if X is playing, 0 if O is playing
    char winner = check_winner(board);
    ...
    if (maximizing_player){
        ...
        for (int i = 0; i < 9; i++) {
            if (board[i] == EMPTY){
                ...
                int eval_score = minimax(board, depth-1, alpha, beta, 0);  //Now the next player is minimizing
                board[i] = EMPTY; //we undo the move
                max_eval = (max_eval > eval_score) ? max_eval : eval_score;
                alpha = (alpha > eval_score) ? alpha : eval_score;  //updating alpha to the best score
        ...
        for (int i = 0; i < 9; i++) {
            if (board[i] == EMPTY){
                ...
                int eval_score = minimax(board, depth-1, alpha, beta, 1);  //Now the next player is maximizing
